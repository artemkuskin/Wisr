import styles from "@/styles/Layout.module.css";
export const Header = () => {
  return (
    <div className={styles.header}>
      <div className={styles.menuButton}>
        <div className={styles.lines}>
          <div className={styles.lineButtons}></div>
          <div className={styles.lineButtons}></div>
          <div className={styles.lineButtons}></div>
        </div>
        <a>
          <img src="data:image/svg+xml;base64," />
        </a>
      </div>
      <div className={styles.headerButtons}>
        <div className={styles.buttonsForPages}>
          <div className={styles.buttonLink}>Loans</div>
          <div className={styles.buttonLink}>Credit Scores</div>
          <div className={styles.buttonLink}>Round Up</div>
          <div className={styles.buttonLink}>Wirs Today</div>
        </div>
        <div className={styles.buttonsDiv}>
          <div className={styles.buttonLogin}>
            <span className={styles.textButtonLogin}>LOG IN</span>
          </div>
          <div className={styles.buttonGet}>
            {" "}
            <span className={styles.textButtonGet}>GET STARTED</span>
          </div>
        </div>
      </div>
    </div>
  );
};
